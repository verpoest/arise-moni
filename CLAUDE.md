...
**eventReceiver duplicate check** (`monitor_health.sh`, section 1c): counts running `eventReceiver` processes (`pgrep -x -c`, falling back to `ps -e -o comm=`) and alerts on **any** deviation from `EVENT_RECEIVER_EXPECTED` (default 7 — one per station, 6 ARISE + IceCube). Too many means an instance was never reaped and two processes are pulling the same stream; too few means a station is not being received at all, and the alert says how many are missing. Note `pgrep -c` prints 0 *and* exits 1 when nothing matches, so the value is validated rather than the exit status.
