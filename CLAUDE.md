...
**eventReceiver duplicate check** (`monitor_health.sh`, section 1c): checks there is exactly one `eventReceiver` per station. Each process is attributed to a station by its `-p` output prefix (`.../data/s5_` -> s5; the IceCube receiver has no prefix and is tagged `icecube`), because a plain process count misses two receivers on one station and none on another — still seven in total. Each receiver's `--server` is additionally checked against that station's `TAXI_IP_N` (`ICECUBE_TAXI_IP` for IceCube), which catches a receiver pointed at the wrong TAXI — one station's data written under another's name, invisible from the output path. Alerts name what is wrong (`s3=missing s5=2(duplicate)`, `s5@192.168.1.112(expected 192.168.1.111)`). Configured by `EVENT_RECEIVER_STATIONS`; stations with no IP configured skip the address check. Note `pgrep -c` prints 0 *and* exits 1 when nothing matches, so the value is validated rather than the exit status.
