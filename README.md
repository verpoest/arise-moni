...
- **WR timestamp check** (`scripts/check_wr_timestamps.py`): Run by both health monitors on the last *completed* binary file of each station, and only once that station's data checks have passed — on a station that is not writing properly the timestamps say nothing and the missing-data alert is the one that matters. Two things are verified: the first White Rabbit (`0x8000`) timestamps must agree with the day and time in the filename (catching a WR on the wrong date or with no lock), and between the two ends of the file the WR seconds must keep step with the free-running RTC sampled in the same blocks (catching a WR that stalls or jumps partway through). Only 20 blocks from each end are read: WR blocks sit roughly every 5 MB, so reading them all would mean reading the whole 18 GiB file twice an hour per station — `--full` does that on demand for a single file. Because the monitors run twice an hour against hourly files, each completed file is checked once and the second look is skipped. A file that contains no WR blocks at all raises its own alert — and blocks that decode to an impossible date are discarded first, so a silent White Rabbit is reported as silent rather than as a clock that drifted. Verdicts use median drift plus a minimum count and fraction of out-of-tolerance blocks, so occasional corrupt words and the known nightly ~20 s WR re-sync stall never page anyone.
